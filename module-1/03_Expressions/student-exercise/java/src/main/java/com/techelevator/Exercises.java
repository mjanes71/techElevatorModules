...
public class Exercises {
	...
	public boolean cigarParty(int cigars, boolean isWeekend) {
		if (isWeekend && (cigars >= 40)) {
			return true;
		}else if (!isWeekend && (40 <= cigars && cigars <= 60)) {
			return true;
		}else {
			return false;
		}
    ...
    public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
        if (isBirthday && ((mealAmount + 5) <= 10)){
        	return "standard";
		} else if (isBirthday && ((mealAmount + 5) <= 15)) {
        	return "special";
		}else if (isBirthday && ((mealAmount + 5) > 15)) {
        	return "ginormous";
