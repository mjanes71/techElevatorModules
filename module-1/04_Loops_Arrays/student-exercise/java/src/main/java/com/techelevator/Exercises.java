...
public class Exercises {
	...
	public boolean firstLast6(int[] nums) {
		boolean firstIndex = nums[0] == 6;
		boolean lastIndex = nums[nums.length - 1] == 6;
		boolean result = firstIndex || lastIndex;
	...
	public boolean sameFirstLast(int[] nums) {
		boolean arrayLength = nums.length >= 1;
		boolean firstAndLastEqual = nums[0] == nums[nums.length - 1];
		boolean result = arrayLength && firstAndLastEqual;
		return result;
	}
	...
	public boolean commonEnd(int[] a, int[] b) {
		boolean firstValueSame = a[0] == b[0];
		boolean lastValueSame = a[a.length - 1] == b[b.length - 1];
		boolean result = firstValueSame || lastValueSame;
		return result;
	}
	...
	public int[] rotateLeft3(int[] nums) {
		int leftMostValue = nums[1];
		int centerValue = nums[2];
		int rightMostValue = nums[0];
		int[] rotatedArray = {leftMostValue, centerValue, rightMostValue};
		return rotatedArray;
	}
	...
	public int[] reverse3(int[] nums) {
		int leftMostValue = nums[2];
		int centerValue = nums[1];
		int rightMostValue = nums [0];
		int[] reverseArray = {leftMostValue, centerValue, rightMostValue};
		return reverseArray;
	}
	...
	public int[] maxEnd3(int[] nums) {
		boolean firstElementLarger = nums[0] > nums[2];
		if (firstElementLarger){
			int[] firstElementWins = {nums[0], nums[0], nums[0]};
			return firstElementWins;
		}else {
			int[] lastElementWins = {nums[2], nums[2], nums[2]};
			return lastElementWins;
		}
	}
	...
	public boolean lucky13(int[] nums) {
		for (int i = 0; i < nums.length; i++){
			boolean oneOrThree = (nums[i] == 1) || (nums[i] == 3);
			if(oneOrThree){
				return false;
			}
		}return true;
