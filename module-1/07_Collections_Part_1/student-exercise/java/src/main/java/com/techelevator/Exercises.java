...
public class Exercises {
	...
	public List<String> no4LetterWords(String[] stringArray) {
		...
		for (String word : stringArray) {
			boolean isNot4Letters = word.length() != 4;
	...
	** INTERVIEW QUESTION **
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		List<Integer> interleaving = new ArrayList<>();

		boolean oneIsLonger = listOne.size() > listTwo.size();

		if (oneIsLonger) {
			for (int i = 0; i < listOne.size(); i++) {
				if (i < listTwo.size()) {
					interleaving.add(listOne.get(i));
					interleaving.add(listTwo.get(i));
				} else {
					interleaving.add(listOne.get(i));
				}
			}
