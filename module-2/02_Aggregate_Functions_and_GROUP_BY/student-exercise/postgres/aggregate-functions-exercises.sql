...
SELECT MAX(population) AS max_world_population
FROM country;
...
SELECT MAX(population) AS largest_city_down_Under
FROM city
...
SELECT name, round((population/surfacearea)::numeric,4) AS population_density, lifeexpectancy 
FROM country
...
SELECT name, gnp-gnpold AS difference, @(gnp-gnpold) AS absolute_difference 
FROM country
...
SELECT continent, round(AVG(lifeexpectancy)::numeric,4) AS average, MIN(lifeexpectancy) AS min_expectancy, MAX(lifeexpectancy) AS max_expectancy 
FROM country
WHERE lifeexpectancy IS NOT NULL
GROUP BY continent
ORDER BY average;
